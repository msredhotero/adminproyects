...
--
-- Servidor: localhost
-- Tiempo de generación: 19-04-2016 a las 17:22:11
...
CREATE TABLE IF NOT EXISTS `audit` (
  ...
  `idmodificado` int(11) DEFAULT NULL,
...
INSERT INTO `audit` (`idaudit`, `tabla`, `idtabla`, `idmodificado`, `previousvalue`, `newvalue`, `dateupdate`, `user`, `action`) VALUES
...
(1, 'Pagina Web', '5000.00', 1, 1, 1589, '20.00', '', NULL),
(4, 'prueba 2', '5000.00', 1, 1, 1356, '20.00', '', b'0');

--
-- Disparadores `proyects`
--
DROP TRIGGER IF EXISTS `project_m_Trigger`;
DELIMITER //
CREATE TRIGGER `project_m_Trigger` AFTER UPDATE ON `proyects`
 FOR EACH ROW BEGIN 

DECLARE sendemailold varchar(1);
DECLARE sendemailnew varchar(1);
...
INSERT INTO audit 
(idaudit, tabla, idtabla, idmodificado, previousvalue, newvalue, dateupdate, user,`action`) 
VALUES ('','proyects', NEW.idproyect, OLD.idproyect, 
CONCAT('(',OLD.title,',', OLD.price,',',OLD.refresponsible,',',OLD.refstate,',', OLD.`order`,',', OLD.commission,',', OLD.observations,',', sendemailold,')'), 
CONCAT('(',NEW.title,',', NEW.price,',',NEW.refresponsible,',',NEW.refstate,',', NEW.`order`,',', NEW.commission,',', NEW.observations,',', sendemailnew,')'), NOW() , user( ), 'update');
END
//
DELIMITER ;
